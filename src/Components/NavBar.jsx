...
export default function NavBar({ item }) {
  ...
  return (
    <div
      className={`w-full sticky top-0 left-0 z-30 bg-white dark:bg-slate-950 dark:text-white  items-center mx-auto p-2 border-b-2 border-slate-200`}
    >
      <div className="flex justify-between p-5 max-w-screen-2xl items-center mx-auto">
        ...
        <div className="order-1 md:order-none flex items-center">
          <ul
            className={`max-w- mx-auto items-center gap-x  md:flex   ${
              !openMenu
                ? "bg-white text-2xl py-4 flex-col w-screen h-screen absolute top-[6.5rem] left-0 z-10 transition-all duration-700 ease-in-out    "
                : "hidden"
            }`}
            onClick={setOpenMenu}
          >
      ...
      <FormSearch 
       ...
       />
          ...
          <div>
            <Link to="/cart">
              {cart.length > 0 ? (
                <span className=" absolute top-7 right- bg-red-600 rounded-full text-white h-5 w-5  text-center text-base">
                  {totalItems}
                </span>
              ) : (
                ""
